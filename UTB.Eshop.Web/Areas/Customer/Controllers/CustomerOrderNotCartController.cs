        ...
        public double AddOrderItemsToSession(int? productId)
        {
            ...
            if (product != null)
            {
                ...
                if (HttpContext.Session.IsAvailable)
                {
                    ...
                    User userUser = EshopDbContext.Users.Include(o => o.MemberName).FirstOrDefault(o => o.UserName == User.Identity.Name);


                    orderItem.Price = orderItem.Product.Price - (orderItem.Product.Price * userUser.MemberName.Discount / 100); 

                    if (orderItemInSession != null)
                    {
                        ++orderItemInSession.Amount;
                        orderItemInSession.Price += orderItem.Product.Price - (orderItem.Product.Price * userUser.MemberName.Discount / 100);   //zde pozor na datový typ -> pokud máte Price v obou případech double nebo decimal, tak je to OK. Mě se bohužel povedlo mít to jednou jako decimal a jednou jako double. Nejlepší je datový typ změnit v databázi/třídě, tak to prosím udělejte.
                    }
                    else
                    {
                    ...
                    totalPrice += orderItem.Product.Price - (orderItem.Product.Price * userUser.MemberName.Discount / 100);
